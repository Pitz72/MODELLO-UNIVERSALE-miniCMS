...
- **SimonePizziWebSite (SPW)** ha il perimetro più maturo: cookie `Secure`, anti session-fixation, invalidazione globale delle sessioni, recupero password completo, IP anti-spoofing.
- **SitoRuntime (SR)** — il sito più ingegnerizzato del trio, quello delle cicatrici di scalabilità — è solo a metà strada: ha ruoli e token CSRF, ma il cookie viaggia senza `Secure`, non rigenera la sessione al login e il suo rate-limit è aggirabile con un header.
- **DISINTELLIGENZA (DIS)** è l'autenticazione "grado zero": niente CSRF, niente rate-limit, cookie ai valori di default di PHP. Eppure è l'unico sito a portare due idee di sicurezza *proprie* e di valore: la difesa anti-frode di un'azione **pubblica** (il voto del festival) e il backup automatico prima di un'operazione distruttiva.

È la dimostrazione più netta della tesi che attraversa tutto il libro: **più ingegnerizzato ≠ più sicuro**, e *più sicuro ≠ più completo su ogni singolo punto*. Il modo giusto di leggere ogni difesa è come una **scala di sottrazione**: cosa resta — e cosa si rompe — quando togli un flag dal cookie, un token da una richiesta, un contatore da un endpoint di login.

> [!NOTE]
> **Una nota di metodo.** Tutti gli stralci di codice di questo capitolo provengono dallo stato reale dei tre siti, citati come `file:linea`. Quando il capitolo dice "il Modello raccomanda" sta prescrivendo; quando dice "SPW fa" / "SR fa" / "DIS fa" sta fotografando il codice in produzione. Le due cose non sempre coincidono — ed è proprio in quello scarto che si impara.

---

## 1. Il perimetro comune: sicurezza fatta a mano

Prima delle divergenze, vale la pena fissare ciò che i tre siti condividono. Sotto le tre implementazioni c'è **lo stesso oggetto**, e ha cinque tratti ricorrenti.
...
**2) Sessione su cookie + `password_verify`: lo zoccolo identico.** Tutti e tre aprono una sessione PHP nativa, cercano l'utente per `username`, verificano la password contro un hash `PASSWORD_DEFAULT`, e popolano `$_SESSION`. Il sistema non conosce mai la password in chiaro:
...
SR scompone la stessa protezione in **mattoni separati** — autorizzazione da una parte, anti-CSRF dall'altra — che ogni endpoint compone a mano, nell'ordine giusto:
...
È più flessibile — il gate a ruolo (`isAdmin()`) è gratis, e arriva quel `admin`/`editor` che SPW non ha — ma è anche **più facile da dimenticare**: la protezione dipende dalla disciplina del singolo endpoint.
...
> In DIS i rami `participants.php?update_status` e `update_round` sono gated **solo** da `isset($_SESSION['user_id'])`, **non** da `isAdmin()`: un *editor* — non un amministratore — può così approvare o respingere partecipanti, mandare le email e spostare i round del festival. La stessa crepa si vede in SR, dove `list_users`/`create_user` stanno *prima* del blocco `401` e si autoproteggono solo con il check di ruolo locale.
> Funziona finché tutti i rami sono scritti con disciplina. Ma un gate unico (`Auth::check()`) **non può** essere dimenticato su un endpoint: o lo includi, o l'endpoint non parte. Questa è la vera differenza tra "middleware" e "convenzione". *(Le conseguenze sul festival si vedono al CAP 18.)*
...
Non richiede alcun handshake col client: il browser invia `Origin`/`Referer` da solo. E poiché vive dentro `Auth::check()`, copre **automaticamente ogni endpoint** che include la guardia.
...
> [!NOTE]
> **Correzione rispetto alla Seconda Edizione.** La vecchia §1.1 presentava come prescrizione un cookie con `cookie_secure = 1` e `cookie_samesite = 'Lax'`. Due cose vanno corrette guardando il codice reale: (a) il `SameSite` effettivo di chi lo imposta è **`Strict`**, non `Lax` (sia SPW sia SR); (b) **solo SPW** imposta davvero tutti e tre i flag.
...
**SPW — i tre flag, in un file condiviso:**
```php
...
**DIS — nessun flag:** il comportamento del cookie dipende interamente dal `php.ini` dell'hosting. Una dipendenza implicita silenziosa.
...
> [!WARNING]
> **I flag del cookie + perché HSTS non è il redirect HTTPS**
> Togliere `Secure` non è un dettaglio cosmetico. Senza `Secure`, il cookie di sessione può viaggiare anche su HTTP in chiaro. Si potrebbe pensare che HSTS protegga comunque — ma **HSTS protegge solo *dopo* la prima visita HTTPS riuscita**. SR applica HSTS ma **non forza** il redirect 301 a HTTPS: c'è una finestra reale di esposizione su HTTP al primo accesso. SPW chiude la finestra da entrambi i lati — `cookie_secure=1` *e* `RewriteCond %{HTTPS} !=on → 301`.
> Lezione: `Secure` sul cookie e redirect HTTPS forzato sono **due difese distinte**; HSTS non sostituisce nessuna delle due.
> Curiosità storica (SPW): fino alla v1.18 i flag stavano *solo* in `auth.php`. Un cookie emesso da un altro endpoint nasceva "debole". La v1.19.0 ha spostato gli `ini_set` nel file condiviso `auth_helper.php`. **La config di sicurezza della sessione va nel file incluso da tutti, non nell'endpoint di login.**
...
> [!NOTE]
> **Correzione rispetto alla Seconda Edizione.** L'esempio precedente leggeva `$_SESSION['username']` come se fosse sempre presente. **In SR non lo è**: il login salva `user_id` e `role` ma **non** `username` (`admin.php:123-124`). Conseguenza concreta: `check_auth` restituisce `username: null`, e il salvataggio articolo ripiega su `author = $_SESSION['username'] ?? 'Admin'`. In SPW e DIS lo `username` è in sessione e l'esempio regge. Non è un buco di sicurezza, ma un'incoerenza di stato reale: lo stesso campo non è garantito su tutti e tre i siti.

---

## 6. Le password e la difesa brute-force

L'hashing è l'unico punto in cui i tre siti sono identici e tutti corretti: `password_hash($pass, PASSWORD_DEFAULT)` alla creazione, `password_verify` alla verifica. Il sistema non conosce mai la password in chiaro, e l'algoritmo può evolvere (bcrypt → argon2) senza toccare il codice.

> [!NOTE]
> **Correzione rispetto alla Seconda Edizione.** La vecchia §3 riduceva la difesa brute-force a un `sleep(1)`. È **incompleto e SR-centrico**. Il `sleep(1)` è solo un accorgimento di SR; la difesa vera è il **lockout** — e soprattutto conta *da quale IP* lo si misura.

La domanda non è "rallentare i tentativi" ma "**dove vive il contatore**". Tre risposte:

- **SPW — tabella DB `login_attempts`.** Dopo 5 tentativi falliti da un IP in 15 minuti → `429`. Al login riuscito il contatore si azzera.
  ```php
  ...
  ```
- **SR — file su disco.** Un JSON per IP in `.cache/ratelimit/<md5(ip)>.json`, finestra 900s, soglia 5, più `sleep(1)` su fallimento. Niente tabella DB (coerente con lo schema MySQL che non contiene `login_attempts`).
- **DIS — niente.** Il login si può martellare quanto si vuole.

> [!TIP]
> **Il contatore brute-force: file vs DB vs assenza**
> Non c'è una sede "giusta" in assoluto. La tabella DB di SPW è transazionale e si presta al riuso (lo vedremo: lo stesso `login_attempts` serve anche il recovery). Il file di SR evita di gravare sul database ma vive fuori dallo schema, quindi non compare in un dump né in una migrazione — è invisibile finché non lo cerchi nel filesystem. L'assenza di DIS è coerente con un sito-festival dove l'admin è uno solo e l'attrito su un login interno è basso — ma resta un'assenza, non una scelta documentata.

Ma il punto più sottile è **da quale variabile leggi l'IP**. È il box-ancora di questo capitolo.
...
> ```
> Quell'header è scritto dal client. Un attaccante lo varia a ogni richiesta e il contatore non si incrementa mai: **il lockout 5/15min è bypassabile**.
>
> **SPW si fida dell'IP TCP, e dell'header solo dietro proxy interno.** `getClientIp()` usa `REMOTE_ADDR` se è già pubblico, e accetta `X-Forwarded-For` (validato `NO_PRIV_RANGE|NO_RES_RANGE`) **solo** quando `REMOTE_ADDR` è privato — cioè solo dietro un proxy fidato. Niente spoofing.
>
...
>
> **La lezione non è "usa sempre `REMOTE_ADDR`".** È che l'IP "giusto" dipende dal **modello d'abuso**: un login da forzare (vuoi che l'attaccante *non possa cambiare* la propria identità → diffida di `X-Forwarded-For`) è il problema opposto di un voto pubblico da non duplicare (stessa esigenza, stessa conclusione: l'header controllato dal client non è affidabile). Lo stesso `REMOTE_ADDR` grezzo è un buco in un contesto e una difesa nell'altro.

---

## 7. `session_version`: invalidare le sessioni a costo zero

Un problema classico dell'auth su sessione: come fai a disconnettere *tutte* le sessioni di un utente — per esempio dopo un reset password — se non tieni uno store server-side delle sessioni attive? **Solo SPW** risolve, con un trucco a costo zero: un intero `session_version` in `users`, copiato in `$_SESSION` al login, e confrontato a ogni richiesta protetta.
...
Incrementando `session_version` di un solo numero (lo fa il reset password, §8), tutte le sessioni con il numero vecchio diventano invalide al primo controllo. Logout-everywhere senza alcuno store di sessioni.

> [!TIP]
> **Fail-closed, non fail-open**
> Il dettaglio che distingue una difesa fatta bene è il ramo `catch`. Se il controllo di `session_version` solleva una `PDOException` (DB irraggiungibile), SPW **nega** l'accesso (`401`), non lo concede. La regola: quando una verifica di sicurezza non può essere completata, l'esito di default deve essere *negato*. SR e DIS non hanno questo meccanismo: in SR un cambio password **non** disconnette le altre sessioni, in DIS nemmeno.

---

## 8. Recovery e reset password fatti bene

È un'intera sezione che il capitolo prima non aveva, perché **solo SPW** implementa il recupero password self-service. SR e DIS hanno solo `change_password` autenticato (devi già essere dentro): password dimenticata = nessun rientro.

Il flusso di SPW ha quattro accortezze che vale la pena isolare.

**1) Token monouso casuale, con scadenza.**
```php
...
**2) Link costruito da URL canonico, non da `HTTP_HOST`.** È la difesa contro il *password-reset poisoning*: se il link nell'email si costruisse da `HTTP_HOST` (controllabile da un header `Host` falsificato), l'attaccante potrebbe far recapitare alla vittima un link che punta al *suo* dominio e intercettare il token.
```php
// public/api/auth.php:227-230
$link = SITE_URL . "/admin/reset-password/{$token}";   // SITE_URL hardcoded, mai HTTP_HOST
```

**3) Enumeration-safe.** La richiesta di recupero risponde **sempre** con lo stesso messaggio generico ("se l'account esiste, riceverai un'email"), che l'utente esista o no — così non si può usare il form per scoprire quali email sono registrate. Lo stesso contatore `login_attempts` viene riusato qui con namespacing della chiave (`'rec:' + sha256(IP)`), per limitare anche gli abusi del recovery.

**4) Il reset invalida le sessioni.** Applicando la nuova password, `session_version` viene incrementato — e per la §7 tutte le sessioni aperte cadono:
```php
...
> - **SPW — random, stampata una volta.** La password del primo admin è generata casualmente e mostrata una sola volta in fase di setup. *(Corretto.)*
> - **SR — hardcoded `runtime2026`.** Il file di manutenzione `fix_users_table.php` ricrea l'utente `admin` con `password_hash('runtime2026', …)` se la tabella è vuota. La password è committata nel repo. La mitigazione reale è il `.htaccess`, che nega l'accesso HTTP a `fix_users_table.php` (§10) — ma **non esiste alcun flusso che obblighi a cambiare la default**: se nessuno usa `change_password`, l'account admin è indovinabile.
> - **DIS — omessa.** Non c'è *nessun* seeding: `init_db.php` ha eliso la creazione dell'admin, e `users.php` può creare utenti solo se sei *già* admin. Risultato: niente default indovinabile (vantaggio), ma il primo admin **vive solo nel file `.sqlite`** e non è ricostruibile dal repo (svantaggio: uovo-e-gallina al primo deploy pulito).
>
> Tre posizioni sulla stessa scala: la random è la più sicura, l'omessa è la più *spartana ma non insicura*, l'hardcoded è l'unica davvero pericolosa — non perché la password esista, ma perché **niente costringe a cambiarla**.
...
**Negare l'accesso diretto al DB-a-file.** DIS tiene il `.sqlite` in una cartella `.data/` generata a runtime e protetta:
```apache
...
```
La regola di base è quella che il capitolo già insegnava — file fuori dalla root pubblica, oppure `Require all denied` + nomi non prevedibili. La novità è che DIS la accoppia a una cartella `.data/` creata *runtime* dal bootstrap (vedi CAP 5).

**Negare gli script di manutenzione — il pattern by-prefix di SR.** È il `.htaccess` più interessante dei tre, perché blocca un'intera *famiglia* di file per prefisso, prima ancora che PHP li veda:
```apache
...
```
È questo deny a rendere *non eseguibile via browser* la `fix_users_table.php` con la password `runtime2026` della §9.

**PHP-off nella cartella upload (SPW).** Un'altra famiglia di file pericolosi sono gli upload: se un attaccante riesce a caricare un `.php`, l'esecuzione va spenta a livello di cartella.
```apache
...
Un errore PHP non gestito che finisce nell'output può rivelare percorsi, query, struttura del database (*path/information disclosure*). Lo standard del Modello: codici HTTP semanticamente corretti, messaggi generici al client, dettaglio tecnico solo nel log.
...
L'ultimo fronte è il più scivoloso: cosa succede quando un'azione è *gated* (richiede login) ma anche *distruttiva*, o quando un'azione è *pubblica* (nessun login) ma deve comunque difendersi dall'abuso.

### 12.1 Il reset a un clic — perché "gated" non basta

In DIS gli endpoint `reset_system.php` e `reset_votes.php` sono gated admin **ma senza CSRF**. Una `POST` cross-site verso `reset_system.php`, innescata mentre l'admin è loggato in un'altra scheda, cancella *tutti* i partecipanti, i voti e gli audio. L'unica mitigazione è il `SameSite` *di default* del cookie (non impostato esplicitamente, §4) — che dipende dalla versione di PHP e non copre ogni caso.

> [!WARNING]
> **Perché un'azione gated ha comunque bisogno del CSRF**
> "È protetta da login" e "è protetta dal CSRF" sono garanzie diverse. Il login dice *chi* sei; il CSRF dice *se sei stato tu a volerlo*. Un'azione distruttiva ha bisogno di entrambe: senza CSRF, è la sessione legittima dell'admin a essere usata contro di lui. Una `confirm` JavaScript ("sei sicuro?") **non** è un sostituto del CSRF — gira sul client, l'attaccante la salta.
...
È esattamente la prevenzione che mancava a SR (CAP 15, "cura senza prevenzione"): il sito più debole sull'identità è l'unico a fare il backup giusto-in-tempo. *Più sicuro ≠ più completo su ogni punto.*

### 12.3 Anti-frode di un'azione pubblica + privacy
...
> Due posture privacy opposte sulla stessa esigenza funzionale. Nota però che l'hash di SPW **non è salato** e usa input a bassa entropia (IP+UA): è anti-collisione, non anonimato crittografico forte — un IP candidato si può ancora verificare per forza bruta. Il confronto pieno tra le due filosofie — *write-time vs render-time*, e identità anonima — è al CAP 20.

Sul versante anti-abuso, vale la pena ricordare che lo stesso `login_attempts` di SPW viene riusato come rate-limit a **due strati** per le reazioni (per-hash 20/min + solo-IP 30/min): il primo strato è aggirabile ruotando lo User-Agent, il secondo è l'argine vero. Anche qui il dettaglio è al CAP 20.

---

## 13. Il lato client e una lezione sui bot

L'area Admin in React è protetta da un `AdminLayout` che, nel suo `useEffect` principale, interroga `check_auth`: se il server risponde `401`, il client **distrugge lo stato locale e reindirizza al login**, impedendo che lampeggi contenuto sensibile. La sidebar e le rotte si generano in base al `role` ricevuto dal server (admin vs editor in SR/DIS).

> [!NOTE]
> **Il client non è la difesa.** Il logout client-side e il check `401` sono *esperienza utente*, non sicurezza: nascondono ciò che non devi vedere, ma è il gate server-side (§2) a impedirti di *toccarlo*. La differenza tra "logout sul client" e l'invalidazione vera via `session_version` (§7) è esattamente questa. L'architettura completa del pannello admin — le tre dashboard, le tre architetture di guardia, il posizionamento dei backup — è il CAP 14.

C'è infine una lezione che proviene da un incidente reale e che resta pertinente qui anche se il caso completo è migrato altrove. Nel febbraio 2026, Runtime Radio è stato sommerso da bot che **simulavano i crawler dei social** (Telegram, Facebook, X) per colpire l'entry-point SEO in PHP, che a ogni richiesta interrogava il database. Lo User-Agent dei bot è falsificabile.

> [!WARNING]
> **Lo User-Agent non è un gatekeeper**
> Non si prendono mai decisioni *di sicurezza* in base allo User-Agent: si può falsificare in un campo header. Lo si può usare per *ottimizzare* (servire una cache ai bot riconosciuti), mai come barriera d'accesso. Il racconto completo dell'attacco DDoS-da-bot e la soluzione (cache statica precompilata, percorso bot separato da quello umano) sono al **CAP 11**, perché il vettore è proprio l'entry-point SEO. Qui resta solo la massima.

---

## In sintesi

La sicurezza è la lente che smentisce l'intuizione "più grande = più robusto". SPW, che non è il sito più ingegnerizzato, ha il perimetro più maturo. SR, il più ricco, lascia tre buchi reali (cookie senza `Secure`, niente anti-fixation, rate-limit aggirabile). DIS, il grado-zero, porta comunque due idee che agli altri mancano (anti-frode pubblica robusta, backup pre-distruttivo). Letta così — come una **scala di sottrazione** — ogni difesa insegna due cose insieme: cosa fa, e cosa si rompe quando la togli.
