...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(13/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **13/14 schede tematiche S1 completate** (S1-C1…C13); resta solo **S1-FORK**. Sessione corrente: blocco C12+C13+FORK.
- **Prossima unità:** FASE 2 / **S1-FORK (FDCA come caso fork/evoluzione)** — fonte FDCA-DIFF. ULTIMA scheda S1: non aggiunge pattern (backend byte-identico a DIS), è il caso-studio del *forking* (re-pelle frontend via AI Studio su backend congelato; debito ereditato immutato — RCE inclusa; frontend↔backend disallineati; v0.0.1 che nasconde il debito v0.5.x; ROADMAP-EVOLUZIONE-miniCMS = il fork che si pianifica da solo). Poi S2/S3/S4. Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
