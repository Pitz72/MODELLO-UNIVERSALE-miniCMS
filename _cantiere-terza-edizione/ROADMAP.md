...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(10/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **10/14 schede tematiche S1 completate** (S1-C1…C5 + S1-C6 Editor + S1-C7 SEO + S1-C8 RSS + S1-C9 Newsletter + S1-C10 Festival; sessione tripla C9/C10/C11 in corso).
- **Prossima unità:** FASE 2 / **S1-C11 Engagement & Reactions** (mono-sito SPW) — fonte SPW-C11 (reactions anonime 5-tipi toggle + voter_hash=SHA256(IP+UA) GDPR-by-design + anti-doppio-voto UNIQUE+INSERT IGNORE + rate-limit a 2 strati; messages.php POST pubblico gated GET/PUT/DELETE + strip_tags write-time = stored-XSS neutralizzato all'origine; due filosofie write-time vs render-time nello stesso codebase). Scheda mono-sito. Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
