...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(8/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **8/14 schede tematiche S1 completate** (S1-C1…C5 + S1-C6 Editor + S1-C7 SEO + S1-C8 RSS; C6+C7 accorpate in una sessione, C8 sola).
- **Prossima unità:** FASE 2 / **S1-C9 Newsletter & Email** (cross-sito) — fonti SPW-C9, SR-C9, DIS-C9 (double opt-in + unsubscribe_token SPW / PHPMailer SMTP + un solo token SR / mail() nativa senza double opt-in né token DIS; CHIUDE definitivamente il quadro dei 4 emettitori del content; rate-limit anti-mail-bombing, header injection, invio sincrono). Valutare se accorpare C9 con C13. Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
