...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(11/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **11/14 schede tematiche S1 completate** (S1-C1…C5 + S1-C6 Editor + S1-C7 SEO + S1-C8 RSS + S1-C9 Newsletter + S1-C10 Festival + S1-C11 Engagement; sessione tripla C9/C10/C11 conclusa).
- **Prossima unità:** FASE 2 / **S1-C12 Admin Dashboard & Panels** (cross-sito) — fonti SPW-C12, SR-C12, DIS-C12 (la dashboard che MISURA con Chart.js SPW / la dashboard che NON misura = console-CRUD SR / via-di-mezzo AdminLayout+guard-componente che misura testuale DIS; backup fuori-docroot SPW vs assente SR; contacts write-only DIS; guard role-blind DIS). Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
