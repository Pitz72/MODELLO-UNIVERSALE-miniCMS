...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(5/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **5/14 schede tematiche S1 completate** (S1-C1 Backend Core, S1-C2 Security & Auth, S1-C3 Frontend Bridge, S1-C4 Content APIs, S1-C5 Media & Upload).
- **Prossima unità:** FASE 2 / **S1-C6 Advanced Editing / Editor** (cross-sito) — fonti SPW-C6, SR-C6, DIS-C6 (Tiptap-blindato SPW / Tiptap + shim migrazione Quill→Tiptap SR / contentEditable+execCommand senza DOMPurify DIS; difesa XSS-stored render-time, guardie link, la scala a 3 gradini dell'editor). Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
