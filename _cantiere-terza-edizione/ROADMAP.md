...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(9/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **9/14 schede tematiche S1 completate** (S1-C1…C5 + S1-C6 Editor + S1-C7 SEO + S1-C8 RSS + S1-C9 Newsletter; sessione tripla C9/C10/C11 in corso).
- **Prossima unità:** FASE 2 / **S1-C10 Festival Logic** (mono-sito DIS; FDCA eredita) — fonte DIS-C10 (macchina a stati iscrizione→selezione→voto→reset, round manuali via flag, vote_count denormalizzato, master switch pubblici, report finale disabilitato, stato finalist vestigiale). Scheda più corta, focalizzata sul "concorso a voto pubblico" come modulo opzionale. Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
