...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(7/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **7/14 schede tematiche S1 completate** (S1-C1…C5 + S1-C6 Editor + S1-C7 SEO; le ultime due accorpate in una sessione perché unite dal filo del "content grezzo").
- **Prossima unità:** FASE 2 / **S1-C8 RSS & Feed** (cross-sito) — fonti SPW-C8, SR-C8, DIS-C8 (file unico news-RSS sicuro-per-sottrazione SPW / trittico feed+proxy-inbound SR / feed podcast iTunes DIS; chiude il "quadro dei 4 emettitori del content": il feed escapa, quindi il buco XSS non si apre). Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
