...
- [ ] S1 — Consolidamento: da card per-sito a **schede tematiche cross-sito** (dedup, pattern comuni, varianti). *(12/14 schede — stato in `sintesi/_INDICE-SINTESI.md`)*
...
- **Fase corrente:** 🟨 **FASE 2 — SINTESI, sotto-fase S1 (Consolidamento) avviata.** FASE 1 conclusa (4 siti, 34 card, copertura completa). FASE 2 in corso: **12/14 schede tematiche S1 completate** (S1-C1…C12; restano S1-C13 e S1-FORK). Sessione corrente: blocco C12+C13+FORK.
- **Prossima unità:** FASE 2 / **S1-C13 DB Evolution & Incidenti** (cross-sito) — fonti SR-C13 (princ.), DIS-C1 (meccanismo update_db_*), SPW-C1 (init fossile). Alto valore e corposo: incidente WAL notturno, migrazione MySQL come reazione, fossili SQLite, 3 schemi subscribers, cura-senza-prevenzione. → CAP 14. Vedi `PROSSIMA-SESSIONE.md`.
- **Log completo:** `LOG.md`.
