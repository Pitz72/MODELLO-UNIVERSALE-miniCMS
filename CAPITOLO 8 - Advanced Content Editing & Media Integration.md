...
> [!NOTE]
> **Correzione rispetto alla Seconda Edizione.** La vecchia §3 descriveva «un editor che rinuncia a pesanti dipendenze esterne in favore di soluzioni native-React». È vero solo per DIS. I due flagship (SPW e SR) usano **Tiptap v3** (ProseMirror), cioè una decina di pacchetti `@tiptap/*`: una dipendenza esterna tutt'altro che leggera, che il capitolo non nominava mai. E la vecchia «Paste Protection», presentata come difesa che rimuove «script e attributi pericolosi», in realtà è solo una pulizia cosmetica dell'incolla. La difesa XSS reale è altrove, e prima non era menzionata. Questo capitolo riallinea entrambe le cose.
